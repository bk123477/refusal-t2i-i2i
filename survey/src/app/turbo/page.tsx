...
export default function TurboEvaluation() {
  ...
  const outputUrl = `${S3_BUCKET_URL}/${currentItem.model}/by_category/${categoryFolder}/${currentItem.filename}_success.png`
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="flex-1 bg-gray-800 rounded-xl overflow-hidden flex items-center justify-center min-h-[400px]">
              <img
                ...
                className="max-w-full max-h-full object-contain"
                onError={(e) => {
                  // Try unchanged version
                  const unchangedUrl = outputUrl.replace('_success.png', '_unchanged.png')
                  if ((e.target as HTMLImageElement).src !== unchangedUrl) {
                    (e.target as HTMLImageElement).src = unchangedUrl
                  }
                }}
              />
            </div>
