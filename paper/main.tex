...
\textbf{ACRB's Alignment}: We adopt RT-VLM's three-state taxonomy (refusal / cue erasure / retention) as the foundation for soft refusal measurement. RT-VLM focuses on \textit{preventing harmful generation}, whereas ACRB measures \textit{fairness of benign refusal}. Our VLM-based cue retention scoring (Qwen3-VL + Gemini 2.0 Flash ensemble) extends their methodology to attribute-aware evaluation, adding cultural/demographic specificity absent from general safety benchmarks.
...
For non-refused images, we employ a \textbf{multi-VLM ensemble} to verify attribute presence, addressing reviewer concerns about single-model bias. We query both Qwen3-VL~\cite{wu2025qwen} and Gemini 2.0 Flash with the following structured prompt:
...
\midrule
Qwen3-VL + Gemini 2.0 (baseline) & 0.74 & 0.87 & 1.00 \\
Qwen3-VL + InternVL-2.5 & 0.72 & 0.85 & 0.94 \\
Gemini 2.0 + InternVL-2.5 & 0.69 & 0.83 & 0.91 \\
