...
1. **초기 필터링**: 각 인종/성별/연령 조합(총 84개 조합)에서 후보 이미지 10-15장씩 추출
2. **품질 평가**: `tools/image_selector/` 웹 애플리케이션을 사용하여 수동 검토
   - 정면 얼굴 여부 확인
   - 이미지 선명도 및 조명 상태 평가
   - 얼굴 특징 식별 가능성 검증
3. **버전별 선별**: V1-V7 버전을 통해 반복적 선별 수행
   - 각 버전마다 7개 인종 × 2개 성별 × 6개 연령대 = 총 84장 선별
   - 품질 기준을 점진적으로 높여 최적의 이미지 확보
4. **결과**: 최종 V4-V7 버전에서 84장 고품질 이미지 확보
